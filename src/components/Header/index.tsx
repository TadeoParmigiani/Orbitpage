...
const Header = () => {
  ...
  const handleScroll = (e, path) => {
    ...
    if (path.startsWith("/#")) {
      const section = document.querySelector(path.replace("/", "")); // Ajustamos el selector correctamente
      if (section) {
        window.scrollTo({
          top: section.offsetTop - 80, // Ajusta según el tamaño del header
          behavior: "smooth",
        });
        setNavbarOpen(false); // Cierra el menú en móviles
      }
    } else {
      window.location.href = path; // Si es una ruta normal, navega
    }
  };
  ...
  return (
    ...
    >
      <div className="container">
        <div className="relative -mx-4 flex items-center justify-between font-extralight">
